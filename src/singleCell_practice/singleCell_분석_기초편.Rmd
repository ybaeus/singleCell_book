...
```{r}
saveRDS(pbmc_alzheimer_SCT.combined, "./rds/pbmc_alzheimer_SCT.combinded_2_clustered_20240302.rds")
```
...
# saveRDS(degs_mildADvsnormal, "./rds/degs_mildADvsnormal.rds")
...
# 각 세포유형 마다 실행하기 
sobj_sebsets_list <- list()
for (celltype in unique(pbmc_alzheimer_SCT.combined$cell_type)){
  ...
  sobj_sebsets_list[[celltype]] <- sobj_subset
}

# 후속 분속에 이용하기 위하여 저장하기 
saveRDS(sobj_sebsets_list, "./rds/sobj_sebsets_list.rds")
```

[코드 10-4] b 세포의 차등 발현 유전자 데이터 확인하기 
```{r}
# b 세포의 차등 발현 유전자 데이터 확인하기 
degs_bcell <- read.csv("./results/B_cell_degs_severeADvsnormal.csv", stringsAsFactors = FALSE)
...
[코드 10-5] B 세포의 차등 발현 유전자 발현 패턴 시각화: 히트맵 
```{r, fig.width = 10}
DoHeatmap(pbmc_alzheimer_SCT.combined, features = degs_bcell$X[1:50]) + noLegend() 
DoHeatmap(sobj_sebsets_list$B_cell, features = degs_bcell$X[1:50])
```

[코드 10-6] b 세포에서의 차등 발현 유전자 박스 플롯 그리기 
```{r}
