...
pbmc_alzheimer_SCT.combinded <- IntegrateData(anchorset = pbmc_alzheimer_SCT.anchors, normalization.method = "SCT") # 메모리가 부족하면 (64GB 미만) 터질 수 있습니다.
# saveRDS(pbmc_alzheimer_SCT.combinded, "./rds/sobj_SCT_merged_method.rds")

```

```{r}
pbmc_alzheimer_SCT.combinded <- readRDS("./rds/pbmc_alzheimer_SCT.combinded_2.rds")  # 2024.01.16 final so far
# (메모용) pbmc_alzheimer_SCT.combinded_1.rds, mt < 5%
# (메모용) pbmc_alzheimer_SCT.combinded_2.rds, mt < 10%, min.cell = 3, min.features = 200, regress=mt
...
```{r}
# 'Run' = 샘플 이름 추가하기 
pbmc_alzheimer_SCT.combinded@meta.data <- pbmc_alzheimer_SCT.combinded@meta.data %>%
...
pbmc_alzheimer_SCT.combinded@meta.data[c("Run", "Age", "subject_status")] <-
  manifest[match(pbmc_alzheimer_SCT.combinded$Run, manifest$Run), ]
```
...
[코드 7-1] PCA 차원 축소 
```{r}
pbmc_alzheimer_SCT.combinded <- RunPCA(pbmc_alzheimer_SCT.combinded, verbose = FALSE)

# Idents(pbmc_alzheimer_SCT.combinded) <-  "orig.ident"
# pdf("./results/pca_results.pdf", width = 10)
print(pbmc_alzheimer_SCT.combinded[["pca"]], dims = 1:10, nfeatures = 5)
DimPlot(pbmc_alzheimer_SCT.combinded, reduction = "pca") + NoLegend() # 나중에 clustering 한 후에, 해당 색칠해서 확인해보면 어느 pc dimension이 어떤 cell type을 구분할 수 있는지도 보인다. 
ElbowPlot(pbmc_alzheimer_SCT.combinded, ndims = 30, reduction = 'pca') # -> 20 dimension 사용 예정
DimHeatmap(pbmc_alzheimer_SCT.combinded, dims = 1:9, cells = 500, balanced = TRUE)
# dev.off()
```
...
```{r}
# TSNE
pbmc_alzheimer_SCT.combinded <- RunTSNE(pbmc_alzheimer_SCT.combinded, reduction = "pca", dims = 1:20, verbose = FALSE)
```

[코드 7-3] UMAP
```{r}
# UMAP
pbmc_alzheimer_SCT.combinded <- RunUMAP(pbmc_alzheimer_SCT.combinded, reduction = "pca", dims = 1:20, verbose = FALSE)
```

[코드 7-4] 클러스터링 
```{r}
pbmc_alzheimer_SCT.combinded <- FindNeighbors(pbmc_alzheimer_SCT.combinded, reduction = "pca", dims = 1:20)
pbmc_alzheimer_SCT.combinded <- FindClusters(pbmc_alzheimer_SCT.combinded, resolution = c(0.1,0.3, 0.5))
```
...
Idents(pbmc_alzheimer_SCT.combinded) <- "integrated_snn_res.0.1"
DimPlot(pbmc_alzheimer_SCT.combinded, label = T, label.size = 7, reduction = "umap")
DimPlot(pbmc_alzheimer_SCT.combinded, label = T, label.size = 7, reduction = "tsne")

Idents(pbmc_alzheimer_SCT.combinded) <- "integrated_snn_res.0.3"
DimPlot(pbmc_alzheimer_SCT.combinded, label = T, label.size = 7, reduction = "umap")
DimPlot(pbmc_alzheimer_SCT.combinded, label = T, label.size = 7, reduction = "tsne")

Idents(pbmc_alzheimer_SCT.combinded) <- "integrated_snn_res.0.5"
DimPlot(pbmc_alzheimer_SCT.combinded, label = T, label.size = 7, reduction = "umap")
DimPlot(pbmc_alzheimer_SCT.combinded, label = T, label.size = 7, reduction = "tsne")
dev.off()
```
...
```{r}
saveRDS(pbmc_alzheimer_SCT.combinded, "./rds/pbmc_alzheimer_SCT.combinded_2_clustered_2024.rds")
pbmc_alzheimer_SCT.combinded <- readRDS("./rds/pbmc_alzheimer_SCT.combinded_2_clustered_2024.rds")
```

[코드 7-5] 배치 효과 확인하기
```{r}
pdf("./results/UMAP_TSNE_by_run.pdf", width = 10)
Idents(pbmc_alzheimer_SCT.combinded) <-  pbmc_alzheimer_SCT.combinded@meta.data[["Run"]]
DimPlot(pbmc_alzheimer_SCT.combinded, label = F, label.size = 7, reduction = "umap")
DimPlot(pbmc_alzheimer_SCT.combinded, label = F, label.size = 7, reduction = "tsne")
dev.off()

pdf("./results/UMAP_TSNE_by_run_split.pdf", width = 15)
Idents(pbmc_alzheimer_SCT.combinded) <-  pbmc_alzheimer_SCT.combinded@meta.data[["Run"]]
DimPlot(pbmc_alzheimer_SCT.combinded, label = F, label.size = 7, reduction = "umap", split.by = "Run")
DimPlot(pbmc_alzheimer_SCT.combinded, label = F, label.size = 7, reduction = "tsne", split.by = "Run")
dev.off()

pdf("./results/UMAP_TSNE_by_age.pdf", width = 10)
Idents(pbmc_alzheimer_SCT.combinded) <-  pbmc_alzheimer_SCT.combinded@meta.data[["Age"]]
DimPlot(pbmc_alzheimer_SCT.combinded, label = F, label.size = 7, reduction = "umap")
DimPlot(pbmc_alzheimer_SCT.combinded, label = F, label.size = 7, reduction = "tsne")
dev.off()
...
Idents(pbmc_alzheimer_SCT.combinded) <-  pbmc_alzheimer_SCT.combinded@meta.data[["Age"]]
DimPlot(pbmc_alzheimer_SCT.combinded, label = F, label.size = 7, reduction = "umap", split.by = "Age")
DimPlot(pbmc_alzheimer_SCT.combinded, label = F, label.size = 7, reduction = "tsne", split.by = "Age")
dev.off()
```
...
# 마커 찾기
Idents(pbmc_alzheimer_SCT.combinded) <- "integrated_snn_res.0.3"
markers <- FindAllMarkers(pbmc_alzheimer_SCT.combinded , only.pos = TRUE) # only positive for now
...
# singleR 이용해서 
# sobj_SCE <- as.SingleCellExperiment(sobj_SCT_merged_method_1, assay="SCT")
sobj_SCE <- as.SingleCellExperiment(pbmc_alzheimer_SCT.combinded, assay="SCT")
results <- SingleR(test = sobj_SCE, ref = ref.data, labels = ref.data$label.main) 
results <- SingleR(test = sobj_SCE, ref = ref.data, labels = ref.data$label.main) 
...
pbmc_alzheimer_SCT.combinded$MonacoImmuneData_main <- results$labels
pbmc_alzheimer_SCT.combinded$MonacoImmuneData_pruned <- results$pruned.labels

Idents(pbmc_alzheimer_SCT.combinded) <- pbmc_alzheimer_SCT.combinded$MonacoImmuneData_pruned # or singleR_label
DimPlot(pbmc_alzheimer_SCT.combinded, label = T, reduction = "umap") 

# pdf("../singleCell_practice/results/UMAP_manacoimmundata.pdf", width = 10)
Idents(pbmc_alzheimer_SCT.combinded) <- pbmc_alzheimer_SCT.combinded$MonacoImmuneData_main # or singleR_label 
DimPlot(pbmc_alzheimer_SCT.combinded, label = T, reduction = "umap") 
# dev.off()
```
