...
[코드 9-3] 알려진 마커들을 이용해서 세포 유형 주석달기 
```{r}
...
for (cluster_n in unique(markers$cluster)){
  ...
  # 각 세포유형 마커들 중 몇개가 매칭되는지 
  total_genes <- length(cluster_markers$gene)
  match_counts <- sapply(paper_markers, function(x) sum(x %in% cluster_markers$gene)) # / total_genes * 100)
...
[코드 9-3] 참조 데이터 Seurat 오브젝트 다운로드 
```{r}
# 다운받을 데이터의 URL 
...
### scType
[코드 9-4] scType에 필요한 툴 및 함수들 다운받기 
```{r}
# 필요한 라이브러리 불러오기  
...
[코드 9-6] scType를 이용한 세포유형 맵핑 시각화 
```{r}
