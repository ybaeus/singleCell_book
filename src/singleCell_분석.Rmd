...
만약 셀 레인저를 실행하기 어려운 환경이라면 https://github.com/ybaeus/singleCell_book 에서 git clone 을 이용해 다운받아주세요.
...
```{r code6_1}
# 필요한 패키지 설치하기
install.packages("Seurat")               # 단일세포 분석 패키지
install.packages("dplyr")                # 데이터 핸들링 패키지
install.packages("hdf5r")                # h5 파일 읽기/쓰기용 패키지
install.packages("future")               # 병렬 처리 패키지
install.packages("BiocManager")          # Bioconductor 패키지 관리
BiocManager::install("glmGamPoi")        # SCTransform의 속도 향상을 돕는 패키지
...
count_SRR13911909.h5 <- Read10X_h5("../data/count_SRR13911909/outs/filtered_feature_bc_matrix.h5")
SRR13911909.h5.sobj_ori <- CreateSeuratObject(counts = count_SRR13911909.h5, project = "pbmc_alzheimer")
print( Sys.time() - start ) # 현재 시간 – 시작 시간 = 총 걸린 시간 (저자 환경: 2.76초)

# [2] Market Exchange Format(MEX) 구조 폴더 이용하기
start <- Sys.time()
count_SRR13911909 <- Read10X(data.dir = "../data/count_SRR13911909/outs/filtered_feature_bc_matrix/")
SRR13911909.sobj_ori <- CreateSeuratObject(counts = count_SRR13911909, project = "pbmc_alzheimer")
print( Sys.time() - start ) # (저자 환경: 6.23초)
```
...
<!-- (책 수록 코드 아님: 참고/실험용) -->
```{r eval=FALSE}
# 콘솔에서 객체 이름 뒤에 @ 를 입력하고 Tab 을 눌러 슬롯 목록을 확인해보세요.
# SRR13911909.h5.sobj@
```
...
saveRDS(lognorm_list, "./rds/lognorm_list.rds")   # [코드 7-4]의 리스트, 챕터 09에서 사용
...
```{r code9_1}
pbmc_alzheimer_SCT.list <- readRDS("./rds/pbmc_alzheimer_SCT.list.rds") # 저장했던 데이터 불러오기
SCT_merged <- merge(pbmc_alzheimer_SCT.list[[1]], y = pbmc_alzheimer_SCT.list[-1],
                    project = "pbmc_alzheimer")
```

(참고)[코드 9-2] 배치 효과 교정 없이 슈랏 오브젝트들 합치기 - 샘플 이름 이용
```{r code9_2}
SCT_merged_named <- merge(pbmc_alzheimer_SCT.list[[1]], y = pbmc_alzheimer_SCT.list[[2]], add.cell.ids = c("SRR13911909", "SRR13911910"), project = "pbmc_alzheimer")
```
...
```{r code9_11}
# [코드 7-9]에서 저장한 로그 정규화 리스트 불러오기
lognorm_list <- readRDS("./rds/lognorm_list.rds")

# 로그 정규화 된 샘플들을 하나의 오브젝트로 합치기  
Lognorm_merged <- merge(lognorm_list[[1]], y = lognorm_list[-1], project = "pbmc_alzheimer")
...
pbmc_alzheimer_avg_expr <- AverageExpression(object = pbmc_alzheimer_SCT.combined, assays = "SCT")

# 결과를 데이터프레임 형태로 변환하기
pbmc_alzheimer_avg_expr <- as.data.frame(t(pbmc_alzheimer_avg_expr$SCT))

# 여섯 샘플에서 값이 모두 같은(분산 0) 유전자는 스케일링할 수 없으므로 제외합니다.
pbmc_alzheimer_avg_expr <- pbmc_alzheimer_avg_expr[, apply(pbmc_alzheimer_avg_expr, 2, var) > 0]

# 평균 발현값을 이용해 주성분 분석(PCA) 수행하기
pbmc_alzheimer_avg_expr_pca <- prcomp(pbmc_alzheimer_avg_expr, center = TRUE, scale = TRUE)
...
# PCA 시각화
ggbiplot(pbmc_alzheimer_avg_expr_pca,
         labels = row.names(pbmc_alzheimer_avg_expr),
         var.axes = FALSE,
...
# DEGs 결과를 CSV 파일로 저장
write.csv(degs_severeADvsnormal, "../results/all_degs_severeADvsnormal.csv", row.names=T)
write.csv(degs_mildADvsnormal, "../results/all_degs_mildADvsnormal.csv", row.names=T)
```
...
for (celltype in unique(pbmc_alzheimer_SCT.combined@meta.data$customclassif)){
  ...
  ## 그룹당 세포가 너무 적으면 차등 발현 검정이 성립하지 않음
  grp <- table(sub$subject_status)
  if (min(grp) < 30) {
    message(glue::glue("{celltype}: skipped (smallest group has only {min(grp)} cells)"))
    next
  }

  ## 중증 환자 vs 일반인  
  ## recorrect_umi = FALSE: [코드 11-3]에서 전체 객체에 PrepSCTFindMarkers()를
  ## 이미 적용했으므로 부분집합에서 다시 보정하지 않음
...
pbmc_alzheimer_SCT.list_processed_doublefinder <- lapply(pbmc_alzheimer_SCT.list_processed, function(sobj) {  
  ...
  # DoubletFinder 실행
  # pK 최적값을 구하는 방법은 [코드 12-2]를 참고하세요.
  sobj_doublefinder <- doubletFinder(sobj, PCs = 1:10, pN = 0.25, pK = 0.09, nExp = nExp_poi, sct = TRUE)
})
```
...
```{r code12_3}
DimPlot(pbmc_alzheimer_SCT.list_processed_doublefinder[[1]],  group.by = "DF.classifications_0.25_0.09_827") 
```

<!-- (책 수록 코드 아님: 참고/실험용) -->
```{r}
# pbmc_alzheimer_SCT.list_processed_doublefinder[[1]] <- subset(pbmc_alzheimer_SCT.list_processed_doublefinder[[1]], subset = DF.classifications_0.25_0.09_827 == "Singlet")
# # TODO 
```
...
```{bash codeCH12_run_cellbender, eval=FALSE}
#!/bin/bash
...
[코드 12-11] 더 많은 해상도를 이용하여 클러스터링 및 클러스트리 실행
```{r code12_11}
# 슈랏 오브젝트 준비시키기
...
#### hdWGCNA
[코드 12-12] hdWGCNA 실행에 필요한 라이브러리 설치 및 불러오기
```{r code12_12}
# 필요한 패키지 설치 및 불러오기  
...
[코드 12-13] hdWGCNA을 이용한 네트워크 분석 1
```{r code12_13}
# 슈랏 오브젝트 불러오기와 설정하기
...
[코드 12-14] hdWGCNA을 이용한 네트워크 분석 2
```{r code12_14}
# 공통발현 네트워크 구축하기
...
[코드 12-15] Slingshot을 이용한 궤적 및 가상 시간 분석
```{r code12_15}
# 필요한 패키지 설치 및 불러오기 
...
[코드 12-16] Slingshot 결과 시각화
```{r code12_16, fig.width = 12}
# UMAP에서 사용할 색깔 지정하기
...
[코드 12-17] 통합 샘플의 Slingshot 궤적 및 가상 시간 분석
```{r code12_17}
# 필요한 패키지 설치 및 불러오기 
BiocManager::install("kstreet13/slingshot")
library(slingshot)
...
[코드 12-21] LIANA 결과 도트 플롯으로 시각화하기
```{r code12_21, fig.wdith=10, fig.height=8}
# 세포 유형 확인하기 
...
#### scriabin
[코드 12-22] scriabin을 이용한 세포 간 소통 상호작용 분석
```{r code12_22}
# 필요한 설치 및 불러오기 
...
pbmc_scriabin_rec <- as.matrix(pbmc_alzheimer_SCT.combined[["IPreceptors"]]@data %>% t() %>% 
                              as.data.frame() %>% add_column(celltype = pbmc_alzheimer_SCT.combined$customclassif) %>% 
                              group_by(celltype) %>% 
...
[코드 12-24] CopyKAT를 이용한 복사수 변이 분석 예시
```{r code12_24}
# 필요한 툴 설치 및 불러오기
...
[코드 12-25] SCEVAN을 이용한 복사수 변이 분석 예시
```{r code12_25}
# 필요한 툴 설치하기
